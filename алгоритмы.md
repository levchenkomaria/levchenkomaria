...
## Ответ на вопрос №3:
Наивная рекурсивная реализация чисел Фибоначчи неэффективна, потому что она многократно выполняет одни и те же вычисления. При таком подходе каждая функция fib(n) вызывает две другие — fib(n-1) и fib(n-2), и каждая из них снова вызывает две свои, и так далее. В результате одни и те же значения вычисляются много раз. Например, чтобы найти fib(5), программа несколько раз заново вычисляет fib(3) и fib(2). Это приводит к тому, что общее количество вызовов растёт экспоненциально, и время работы алгоритма имеет сложность порядка O(2ⁿ). Кроме того, при глубокой рекурсии используется дополнительная память под каждый уровень вызова функции. Таким образом, наивная рекурсия крайне неэффективна по времени. Эту проблему можно решить, если запоминать уже вычисленные значения (использовать мемоизацию) или заменить рекурсию на итеративный способ вычисления, при котором каждое число Фибоначчи вычисляется только один раз.
