...
## Временная сложность:
 ...
 • Средний случай: O(n) — если элемент находится где-то в середине массива или отсутствует.
 • Худший случай: O(n) — если элемент отсутствует или находится в конце массива.

## Почему такая сложность:
 • В худшем случае алгоритм проверяет все n элементов массива, что даёт линейную зависимость от размера массива.
...
## Временная сложность:
 • Лучший случай: O(1) — если искомый элемент находится в середине массива на первом шаге.
 • Средний и худший случай: O(log n) — каждый шаг уменьшает область поиска вдвое.

## Почему O(log n):
 • На каждом шаге алгоритм делит оставшийся массив пополам.
 • Для массива из n элементов количество делений до нахождения элемента или окончания поиска примерно равно log₂(n).
...
## Временная сложность:
 ...
 • Средний случай: O(log log n) — для равномерно распределенных массивов.
 • Худший случай: O(n) — если массив неравномерно распределен, а элемент находится в конце или отсутствует.

## Почему такая сложность:
 • Интерполяционный поиск быстрее бинарного при равномерном распределении, потому что вычисляет приблизительную позицию элемента.
 • Если распределение элементов неравномерное, алгоритм может деградировать до последовательного поиска.
...
## Временная сложность:
 • Лучший случай: O(1) — если элемент сразу находится на предполагаемой позиции.
 • Средний и худший случай: O(log n) — количество сравнений ограничено числами Фибоначчи, аналогично бинарному поиску.

## Почему O(log n):
 • На каждом шаге поиск сокращает рассматриваемый диапазон примерно на число Фибоначчи, что даёт логарифмическую зависимость от размера массива.
 • Алгоритм особенно эффективен при работе с медленным доступом к памяти, так как уменьшает количество сравнений.
